...
function hasSheetsConfig(): boolean {
  return Boolean(env.GOOGLE_SHEET_ID && env.GOOGLE_SERVICE_ACCOUNT_KEY);
}
...
async function getSheetsClient() {
  const credentials = parseServiceAccountCredentials(env.GOOGLE_SERVICE_ACCOUNT_KEY);
  ...
  try {
    ...
    if (existingRowIndex >= 0) {
      await sheets.spreadsheets.values.update({
        spreadsheetId: env.GOOGLE_SHEET_ID,
        range: `${SHEET_TAB_NAME}!A${existingRowIndex + 1}`,
